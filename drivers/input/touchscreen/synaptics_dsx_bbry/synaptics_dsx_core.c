...
#define EXP_FN_WORK_DELAY_MS 0 /* ms */
...
//Send BTN_MASK when CKB swipes occur to mask inadvertent touch on Home/Back Navigation keys
#define IGNORE_TURN_OFF_FLAG false /* Keep CKB ON to mask Nav events */
#ifdef CONFIG_CKB_MASK_KEY
#define MASK_Y_SWIPE_LENGTH 90
#define MAX_X_LENGTH 75
#define MIN_Y_MOVEMENT 3
#define BTN_MASK 454
int mask_on = 0;
int swipe_start_y_location[10];
...
struct synaptics_rmi4_data *data_global;
...
static ssize_t synaptics_rmi4_full_raw_rt78_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_open_rt78_show(struct device *dev,
		struct device_attribute *attr, char *buf);

#endif /*CONFIG_BBRY_MFG || CONFIG_BBRY_DEBUG*/

/* MODIFIED-BEGIN by Haojun Chen, 2016-12-09,BUG-3654877*/
static ssize_t synaptics_rmi4_config_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);
		/* MODIFIED-END by Haojun Chen,BUG-3654877*/
...
static ssize_t synaptics_rmi4_pointer_type_show(struct device *dev,
                struct device_attribute *attr, char *buf);
static ssize_t synaptics_rmi4_pointer_type_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(turn_off, (S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP),
        ...
        __ATTR(pointer_type, (S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP),
                        synaptics_rmi4_pointer_type_show,
                        synaptics_rmi4_pointer_type_store),
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_DDT
	...
	__ATTR(BIST, (S_IRUSR | S_IRGRP),
			...
			synaptics_rmi4_store_error),
	__ATTR(full_raw_rt78, (S_IRUSR | S_IRGRP),
			synaptics_rmi4_full_raw_rt78_show,
			synaptics_rmi4_store_error),
	__ATTR(open_rt78, (S_IRUSR | S_IRGRP),
			synaptics_rmi4_open_rt78_show,
			synaptics_rmi4_store_error),
#endif /*CONFIG_BBRY_MFG || CONFIG_BBRY_DEBUG*/
			...
			struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->stats.active) {
		...
		add_timer(&rmi4_data->stats.timer);
		dev_dbg(rmi4_data->pdev->dev.parent,
			"Timer expires in %d msecs\n", jiffies_to_msecs(rmi4_data->stats.timer.expires));
	}
	mutex_unlock(&rmi4_data->stats.mutex);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	size += snprintf(buf+strlen(buf), PAGE_SIZE - strlen(buf),
		"irq=%d, irq_enabled=%d, irq_gpio=%d\n",
		rmi4_data->irq,
                ...
                struct device_attribute *attr, char *buf)
{
        struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
        return snprintf(buf, PAGE_SIZE, "%u\n",
                (strcmp(rmi4_data->input_dev->name, "touch_keypad_pointer") == 0));
}
...
{
        struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
        if (strcmp(rmi4_data->hw_if->board_data->input_dev_name, "touch_keypad") == 0) {
                unsigned int needsPointerMode;
                bool isPointerMode;
                if (sscanf(buf, "%u", &needsPointerMode) != 1)
                        needsPointerMode = 0;
                isPointerMode = (strcmp(rmi4_data->input_dev->name,
                        "touch_keypad_pointer") == 0);
                if ((needsPointerMode == 1) != isPointerMode) {
                        /* Cancel any existing touches and force a reset of */
                        /* the current device in upper layers through sync  */
                        /* drop.                                            */
                        synaptics_rmi4_free_fingers(rmi4_data);
                        input_event(rmi4_data->input_dev, EV_SYN, SYN_DROPPED,
                                0);
                        input_sync(rmi4_data->input_dev);
                        if (rmi4_data->secondary_input_dev == NULL) {
                                /* Should be non-pointer as initial config is */
                                /* is always expected to take the non-pointer */
                                /* name. */
                                if (isPointerMode) {
                                        dev_err(rmi4_data->pdev->dev.parent,
                                                "%s: Unexpected pointer device only\n",
                                                __func__);
                                } else {
                                        /* Define and register a new pointer device,   */
                                        /* which will differ only in the name. The IDC */
                                        /* file lookup is based on the device name.    */
                                        rmi4_data->secondary_input_dev = rmi4_data->input_dev;
                                        if (synaptics_rmi4_set_input_dev(rmi4_data) < 0) {
                                                dev_err(rmi4_data->pdev->dev.parent,
                                                        "%s: Failed to add pointer input device.\n",
                                                        __func__);
                                                rmi4_data->input_dev = rmi4_data->secondary_input_dev;
                                                rmi4_data->secondary_input_dev = NULL;
                                        } else
                                                rmi4_data->input_dev->name = "touch_keypad_pointer";
                                }
                        } else {
                                /* The second device is defined; just swap them. */
                                struct input_dev *input_dev_temp =
                                        rmi4_data->secondary_input_dev;
                                rmi4_data->secondary_input_dev = rmi4_data->input_dev;
                                rmi4_data->input_dev = input_dev_temp;
                        }
                }
        }
        return count;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!f54) {
		...
		return snprintf(buf, PAGE_SIZE,
			"BIST TEST FAILED, NO F54\n");
	}

	rx = 9;//f54->rx_assigned;
	tx = 4;//f54->tx_assigned;
	...
	report_size = 2 * rx * tx;
	testdata = kmalloc(report_size/2, GFP_KERNEL);
	if(NULL == testdata)
		return snprintf(buf, PAGE_SIZE,
			"BIST TEST FAILED\n");
	...
	if (retval) {
		...
		return snprintf(buf, PAGE_SIZE, "BIST TEST COMPLETE\n"
			...
			testdata[16],testdata[17],testdata[18],testdata[19],
			testdata[20],testdata[21],testdata[22],testdata[23],
			testdata[24],testdata[25],testdata[26]);
		}
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	testdata = kzalloc(2 * rx_num, GFP_KERNEL);
	if(NULL == testdata)
		return snprintf(buf, PAGE_SIZE,
			"BIST TEST FAILED\n");
	
	retval = synaptics_rmi4_f54_do_full_raw_rt78(rmi4_data, testdata);
	if (retval < 0)
		return snprintf(buf, PAGE_SIZE, "fail to read report image 78\n");

	if (!testdata)
		return snprintf(buf, PAGE_SIZE, "invalid testdata\n");

	rt_data_16 = (unsigned short *)testdata;
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = synaptics_rmi4_f54_do_open_rt78(rmi4_data, testdata);
	if (retval < 0)
		return snprintf(buf, PAGE_SIZE, "fail to read report image 78\n");
...
extern int fwu_get_device_config_id(struct synaptics_rmi4_data *rmi4_data);
static ssize_t synaptics_rmi4_config_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	...
	if (retval < 0) {
		return snprintf(buf, PAGE_SIZE, "read config_id failed\n");
	} else {
	...
	unsigned char   finger)
{
 ...
 /**
 * synaptics_rmi4_force_cal(
 *
 * This function forces a new touch baseline
 */
static int synaptics_rmi4_force_cal(
	struct synaptics_rmi4_data *rmi4_data)
{
	unsigned char command;
	int retval;
...
/**
* synaptics_rmi4_force_cal_work()
*
* Called by the kernel at the scheduled time.
*
* This function is a work thread that forces new baseline after a resume.
*/
static void synaptics_rmi4_force_cal_work(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct synaptics_rmi4_data *rmi4_data =
		container_of(dw, struct synaptics_rmi4_data, force_cal_work);
	...
	bool report)
{
	...
	if (rmi4_data->resume_ignore_touch) {
		// ignore touch for delay time (do not reset delay if touch occured)
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			// Scale Y
			//y = BBRY_SCALE_Y_POS(y, 0, rmi4_data->sensor_max_y, 20, 304);
			if (y < 0) {
				y = 0;
			}
			...
			average_y_movement[finger] = (average_y_movement[finger] + y_movement)/2;
			x_swipe_length = abs(swipe_start_x_location[finger] - x);
			y_swipe_length = swipe_start_y_location[finger] - y;

			dev_dbg(rmi4_data->pdev->dev.parent,
				"CKB_SwipeCheck(%d),lasty=%d,y=%d,y_movement=%d,average_y_movement=%d, swipe_start_y_location=%d,y_swipe_length=%d, swipe_start_x_location=%d,x_swipe_length=%d.",finger,last_y_location[finger],y,y_movement,average_y_movement[finger],swipe_start_y_location[finger],y_swipe_length,swipe_start_x_location[finger],x_swipe_length);

			last_y_location[finger] = y;

			if (swipe_start_y_location[finger]<y){
				swipe_start_y_location[finger] = y;
				swipe_start_x_location[finger] = x;
				average_y_movement[finger] = 0;
				dev_dbg(rmi4_data->pdev->dev.parent,"CKB_SwipeCheck(%d) reset start position Y increasing",finger);
			}
			if (x_swipe_length>MAX_X_LENGTH){
				swipe_start_y_location[finger] = y;
				swipe_start_x_location[finger] = x;
				average_y_movement[finger] = 0;
				dev_dbg(rmi4_data->pdev->dev.parent,"CKB_SwipeCheck(%d) reset start position X moved too far",finger);
			}
			if (average_y_movement[finger]<MIN_Y_MOVEMENT){
				swipe_start_y_location[finger] = y;
				swipe_start_x_location[finger] = x;
				average_y_movement[finger] = 0;
				dev_dbg(rmi4_data->pdev->dev.parent,"CKB_SwipeCheck(%d) reset start position Y stopped moving",finger);
			}
		...
		case F12_PALM_STATUS:
			...
			dev_info(rmi4_data->pdev->dev.parent,
				"CKB Noise Mitigation - forcing new baseline\n");
			synaptics_rmi4_force_cal(rmi4_data);
...
// extrapolate Y
#if 0//def CKB_MASK_KEY // MODIFIED by Haojun Chen, 2016-12-22,BUG-3824354
#ifdef TYPE_B_PROTOCOL
			input_mt_slot(rmi4_data->input_dev, finger);
			input_mt_report_slot_state(rmi4_data->input_dev,
					MT_TOOL_FINGER, 1);
#endif
			y = last_y_location[finger] - average_y_movement[finger];
			if (y < 0) {
				y = 0;
			}
			if (y > rmi4_data->sensor_max_y) {
...
static int synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (status.unconfigured && !status.flash_prog) {
		...
		if (retval < 0) {
			dev_err(dev, "%s: Failed to reinit device\n", __func__);
			return retval;
		}
	}
		...
		list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
			if (fhandler->num_of_data_sources) {
				if (fhandler->intr_mask & intr[fhandler->intr_reg_num]) {
					retval = synaptics_rmi4_report_touch(rmi4_data, fhandler, true);
					if (retval < 0) {
						dev_err(dev, "%s: Failed to report " "touch\n", __func__);
						return retval;
					}
...
static int synaptics_rmi4_query_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			dev_dbg(rmi4_data->pdev->dev.parent,
					"%s: F%02x found (page %d), intr_src_count=%d\n",
					__func__, rmi_fd.fn_number,
					page_number, rmi_fd.intr_src_count);
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (bdata->power_gpio >= 0) {
		...
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to configure power GPIO. keep going\n",
					__func__);
			retval = 0;
 		}
...
#if 0 /* BBRY:TODO: need to revisit */
err_gpio_power:
#endif
	synaptics_rmi4_gpio_setup(
			bdata->irq_gpio,
...
static int synaptics_rmi4_free_fingers(struct synaptics_rmi4_data *rmi4_data)
{
	...
	memset(average_y_movement, 0, sizeof(average_y_movement));
#endif
#ifdef TYPE_B_PROTOCOL
...
/**
* synaptics_bbry_rmi4_new_function()
*
...
* respectively.
*/
void synaptics_bbry_rmi4_new_function(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_exp_fn *exp_fn,
		bool insert)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	rmi4_data = data_global;
...
EXPORT_SYMBOL(synaptics_bbry_rmi4_new_function);
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if (!hw_if) {
		dev_err(&pdev->dev, "%s: No hardware interface found\n", __func__);
		return -EINVAL;
	}

	bdata = hw_if->board_data;
	if (!bdata) {
		dev_err(&pdev->dev, "%s: No board data found\n", __func__);
		return -EINVAL;
	}

	rmi4_data = kzalloc(sizeof(*rmi4_data), GFP_KERNEL);
	if (!rmi4_data) {
		dev_err(&pdev->dev, "%s: Failed to alloc mem for rmi4_data\n", __func__);
		return -ENOMEM;
	}

	data_global = rmi4_data;
	rmi4_data->pdev = pdev;
	rmi4_data->do_once = 1;
	if (bdata->wg_enabled) {
	...
	if (bdata->reg_en) {
		...
		rmi4_data->regulator_2p8v.regulator = bdata->vcc_2p8v;
		...
		retval = synaptics_regulator_enable(rmi4_data, &rmi4_data->regulator_2p8v);
		if (retval) {
			dev_err(&pdev->dev,
				"%s: Error with vcc\n",
				__func__);
	...
	if (bdata->power_2p8_en >= 0) {
		dev_err(&pdev->dev,
			"%s: Configuring power_2p8_en GPIO %d\n",
			__func__, bdata->power_2p8_en);
		retval = synaptics_rmi4_gpio_setup(
				bdata->power_2p8_en,
				true, 1, 1);
		if (retval < 0) {
			dev_err(&pdev->dev,
			"%s: Failed to configure power_2p8_en GPIO %d, ignoring the error\n",
			__func__, retval);
			retval = 0;
		}
		usleep(1000);
	...
	if (bdata->power_1p8_en >= 0) {
		dev_err(&pdev->dev,
			"%s: Configuring power_1p8_en GPIO %d\n",
			__func__, bdata->power_1p8_en);
		retval = synaptics_rmi4_gpio_setup(
				bdata->power_1p8_en,
				true, 1, 1);
		if (retval < 0) {
			dev_err(&pdev->dev,
			"%s: Failed to configure power_1p8_en GPIO %d, ignoring the error\n",
			__func__, retval);
			retval = 0;
		}
		usleep(1000);
	...
	if (bdata->main_power_gpio >= 0) {
		...
		if (retval < 0) {
			dev_err(&pdev->dev,
			"%s: Failed to configure main power GPIO %d, ignoring the error\n",
			__func__, retval);
			retval = 0;
		}
		usleep(bdata->power_delay_ms * 1000);
	...
	INIT_DELAYED_WORK(&rmi4_data->force_cal_work,
				synaptics_rmi4_force_cal_work);
...
#if 1 /* BBRY: TODO, FB notify doesn't seems to work. For now trigger a reset */
        queue_work(rmi4_data->workqueue, &rmi4_data->reset_work);
#endif
...
static int synaptics_rmi4_remove(struct platform_device *pdev)
{
	...
	rmi4_data->input_dev = NULL;
        ...
        if (rmi4_data->secondary_input_dev != NULL) {
                input_unregister_device(rmi4_data->secondary_input_dev);
                rmi4_data->secondary_input_dev = NULL;
        }
		...
		struct synaptics_rmi4_data *rmi4_data)
{
	...
	turn_off = rmi4_data->turn_off && !IGNORE_TURN_OFF_FLAG;
...
void synaptics_bbry_rmi4_glass_on_notif(bool glass_off)
{
...
EXPORT_SYMBOL(synaptics_bbry_rmi4_glass_on_notif);
		...
		bool enable)
{
	...
	if (enable) {
		...
		if (bdata->power_2p8_en >= 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Enabling power_2p8_en gpio\n",
					__func__);
			gpio_set_value(bdata->power_2p8_en,
					1);
			usleep(bdata->power_delay_ms * 1000);
		}
...
#if 0 /*Do not turn off power rails or hold touch IC in reset*/
	if (!bdata->pm_disabled) {
		retval = synaptics_rmi4_power_enable(rmi4_data, false);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
...
#endif // MODIFIED by Haojun Chen, 2016-12-19,BUG-3705350
    return 0;
}
 /**
...
int synaptics_rmi4_runtime_suspend(struct device *dev)
{
	...
	cancel_delayed_work_sync(&rmi4_data->force_cal_work); // MODIFIED by Haojun Chen, 2016-12-06,BUG-3647345
...
static int __init synaptics_rmi4_init(void)
{
	...
	retval = synaptics_bbry_rmi4_bus_init();
	if (retval)
		return retval;
