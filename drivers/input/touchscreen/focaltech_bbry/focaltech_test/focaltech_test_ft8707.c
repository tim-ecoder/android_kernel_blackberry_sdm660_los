...
#define FT8707_LEFT_KEY_REG    0X1E
#define FT8707_RIGHT_KEY_REG   0X1F
...
boolean FT8707_StartTest(void)
{
	...
	for(iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
	{
		...
		///////////////////////////////////////////////////////FT8707_ENTER_FACTORY_MODE
		if(Code_FT8707_ENTER_FACTORY_MODE == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{			
			ReCode = FT8707_TestItem_EnterFactoryMode();
		...
		///////////////////////////////////////////////////////FT8707_CHANNEL_NUM_TEST
		if(Code_FT8707_CHANNEL_NUM_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{
			ReCode = FT8707_TestItem_ChannelsTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
		...
		if(Code_FT8707_RAWDATA_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{
			ReCode = FT8707_TestItem_RawDataTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
		...
		///////////////////////////////////////////////////////FT8707_NOISE_TEST
		if(Code_FT8707_NOISE_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{

			ReCode = FT8707_TestItem_NoiseTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
		...
		if(Code_FT8707_CB_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{
			ReCode = FT8707_TestItem_CbTest(&bTempResult); //
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
...
***********************************************************************/
int FT8707_get_test_data(char *pTestData)
{
	if(NULL == pTestData)
	{
...
static int StartScan(void)
{
	...
	const unsigned char MaxTimes = 20;	//ȴ160ms
...
static unsigned char GetTxRxCB(unsigned short StartNodeNo, unsigned short ReadNum, unsigned char *pReadBuffer)
{
	...
	unsigned short usReturnNum = 0;//ÿҪصĸ
	unsigned short usTotalReturnNum = 0;//ܷظ
	...
	for(i = 1; i <= iReadNum; i++)
	{
		...
		wBuffer[1] = (StartNodeNo+usTotalReturnNum) >>8;//ַƫ8λ
		wBuffer[2] = (StartNodeNo+usTotalReturnNum)&0xff;//ַƫ8λ
...
/************************************************************************
* Name: FT8707_TestItem_EnterFactoryMode
...
{	
	...
	if(ReCode == ERROR_CODE_OK)	//ģʽɹ󣬾Ͷͨ
	{	
...
static unsigned char GetChannelNum(void)
{
	...
	for(i = 0; i < 3; i++)
	{
		...
		g_stSCapConfEx.KeyNum = 0;
		ReCode = ReadReg( FT8707_LEFT_KEY_REG, &regData );
		if(ReCode == ERROR_CODE_OK)
		{
		...
		ReCode = ReadReg( FT8707_RIGHT_KEY_REG, &regData );
		if(ReCode == ERROR_CODE_OK)
		{
...
/************************************************************************
* Name: FT8707_TestItem_ChannelsTest
* Brief:  Check whether TP can enter Factory Mode, and do some thing
* Input: none
* Output: none
* Return: Comm Code. Code = 0x00 is OK, else fail.
...
unsigned char FT8707_TestItem_ChannelsTest(bool * bTestResult)
{
	...
	if(ReCode == ERROR_CODE_OK)
	{
		if((g_stCfg_FT8707_BasicThreshold.ChannelNumTest_ChannelXNum == g_stSCapConfEx.ChannelXNum)
			&& (g_stCfg_FT8707_BasicThreshold.ChannelNumTest_ChannelYNum == g_stSCapConfEx.ChannelYNum)
			&& (g_stCfg_FT8707_BasicThreshold.ChannelNumTest_KeyNum == g_stSCapConfEx.KeyNum))
		{
			* bTestResult = true;
			FTS_TEST_DBG("\n\nGet channels: (CHx: %d, CHy: %d, Key: %d), Set channels: (CHx: %d, CHy: %d, Key: %d)",
				...
				g_stCfg_FT8707_BasicThreshold.ChannelNumTest_ChannelXNum, g_stCfg_FT8707_BasicThreshold.ChannelNumTest_ChannelYNum, g_stCfg_FT8707_BasicThreshold.ChannelNumTest_KeyNum);
...
/************************************************************************
* Name: FT8707_TestItem_RawDataTest
...
/************************************************************************
* Name: FT8707_TestItem_NoiseTest
...
unsigned char FT8707_TestItem_NoiseTest(bool* bTestResult)
{
	...
	iNoiseFrames = g_stCfg_FT8707_BasicThreshold.NoiseTest_Frames;
	...
	if(NT_AvgData == g_stCfg_FT8707_BasicThreshold.NoiseTest_NoiseMode)
	{
	...
	else if(NT_MaxData == g_stCfg_FT8707_BasicThreshold.NoiseTest_NoiseMode)
	{
	...
	else if(NT_MaxDevication == g_stCfg_FT8707_BasicThreshold.NoiseTest_NoiseMode)
	{
	...
	else if(NT_DifferData == g_stCfg_FT8707_BasicThreshold.NoiseTest_NoiseMode)
	{
		//Caculate the Avg Value of all nodes
		memset(m_NoiseData, 0, sizeof(m_NoiseData));
	...
	if( ReCode != ERROR_CODE_OK ) goto TEST_ERR;
		...
		iMinValue = 0;
		iMaxValue = g_stCfg_FT8707_BasicThreshold.NoiseTest_Coefficient * chNoiseValue * 32 / 100;
		FTS_TEST_DBG("");
		for(iRow = 0;iRow < (g_stSCapConfEx.ChannelXNum + 1);iRow++)
		{
			for(iCol = 0;iCol < g_stSCapConfEx.ChannelYNum;iCol++)
...
unsigned char FT8707_TestItem_CbTest(bool* bTestResult)
{
	...
	iMinValue = g_stCfg_FT8707_BasicThreshold.CbTest_Min;
	iMaxValue = g_stCfg_FT8707_BasicThreshold.CbTest_Max;	
	for(iRow = 0;iRow < (g_stSCapConfEx.ChannelXNum + 1);iRow++)
	{
		for(iCol = 0;iCol < g_stSCapConfEx.ChannelYNum;iCol++)
		{
...
unsigned char FT8707_GetTestResult(void)
{
	...
	for(iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
	{
		...
		///////////////////////////////////////////////////////Code_FT8707_WEAK_SHORT_CIRCUIT_TEST
		if(Code_FT8707_SHORT_CIRCUIT_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{
			if(RESULT_PASS == g_stTestItem[ucDevice][iItemCount].TestResult)
