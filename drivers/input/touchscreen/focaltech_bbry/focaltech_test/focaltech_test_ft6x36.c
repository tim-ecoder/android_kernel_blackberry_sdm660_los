...
#define C6X36_CHANNEL_NUM	0x0A	//1 Byte	д(RW)	TP_Channel_Num		VAͨ ֵΪ63
#define C6X36_KEY_NUM		0x0B	//1 Byte	д(RW)	TP_Key_Num		ⰴͨ ֵΪ63

#define C6X36_CB_ADDR_W 0x32/*0x32*/	//1 Byte	д(RW)	CB_addr		ͨģʽCB--ַ		
#define C6X36_CB_ADDR_R 0x33/*0x32*/	//1 Byte	д(RW)	CB_addr		ͨģʽCB--ַ	
#define C6X36_CB_BUF  0x39/*0x33*/	//1 Byte	ֻ(RO)	CB_buf		һͨӦ2ֽڣ˳ 2*N							
#define C6X36_RAWDATA_ADDR	0x34/*0x34*/	//1 Byte	д(RW)	RawData_addr		Rawdata--ַ							
#define C6X36_RAWDATA_BUF	0x35

#define C6206_FACTORY_TEST_MODE			0xAE	//0ģʽ,1Թģʽ1ʹõ+0ˮɨ裩,2Թģʽ2ʹõ+ˮɨ裩
...
static int StartScan(void)
{
	unsigned char RegVal = 0x01;
    	...
    	const unsigned int MaxTimes = 500/*20*/;	//ȴ160ms
...
{	
	...
	if(ReCode == ERROR_CODE_OK) //ȡRawDataͼDifferֵ
	{
		FTS_TEST_DBG("\r\n//======= Test Data:  ");
		ShowRawData();
	}
	...
	{
		RawDataMin = g_stCfg_SCap_DetailThreshold.RawDataTest_Min[i];//ȡϸֵ
		RawDataMax = g_stCfg_SCap_DetailThreshold.RawDataTest_Max[i];//ȡϸֵ
...
unsigned char FT6X36_TestItem_CbTest(bool * bTestResult)
{
	...
	else
	{
		...
		for(i = 0; i < g_ScreenSetParam.iChannelsNum + g_ScreenSetParam.iKeyNum; i++)
		{
			...
			iCbMin = g_stCfg_SCap_DetailThreshold.CbTest_Min[i];//ȡϸֵ
			iCbMax = g_stCfg_SCap_DetailThreshold.CbTest_Max[i];//ȡϸֵ
...
unsigned char FT6X36_TestItem_DeltaCbTest(unsigned char * bTestResult)
{
	...
	//GetDeltaCiDataMsg();//ռDelta Ci DataCSVļ
	return 0;
}
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 1)
		{
			...
			else
			{
				if(Sort1LastNum + 1 == i)//ͨ
				{
					...
					//{//֣ͨǰ벿ֵĽβ벿ֵĿʼȽϣעͨ0ʼǰ벿ֵĽβg_ScreenSetParam.iChannelsNum/2 - 1
					//}
