...
boolean FTE716_StartTest(void)
{
	...
	for(iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
	{
		...
		if(Code_FTE716_ENTER_FACTORY_MODE == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{			
			ReCode = FTE716_TestItem_EnterFactoryMode();
		...
		///////////////////////////////////////////////////////FTE716_CHANNEL_NUM_TEST
		if(Code_FTE716_CHANNEL_NUM_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{
			ReCode = FTE716_TestItem_ChannelsTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
		...
		if(Code_FTE716_RAWDATA_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{
			ReCode = FTE716_TestItem_RawDataTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
		...
		///////////////////////////////////////////////////////FTE716_NOISE_TEST
		if(Code_FTE716_NOISE_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{

			ReCode = FTE716_TestItem_NoiseTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
		...
		if(Code_FTE716_CB_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{
			ReCode = FTE716_TestItem_CbTest(&bTempResult); //
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
...
/************************************************************************
* Name: FTE716_TestItem_EnterFactoryMode
...
static unsigned char GetChannelNum(void)
{
	...
	for(i = 0; i < 3; i++)
	{
		...
		g_stSCapConfEx.KeyNum = 0;
		ReCode = ReadReg( FTE716_LEFT_KEY_REG, &regData );
		if(ReCode == ERROR_CODE_OK)
		{
		...
		ReCode = ReadReg( FTE716_RIGHT_KEY_REG, &regData );
		if(ReCode == ERROR_CODE_OK)
		{
...
/************************************************************************
* Name: FTE716_TestItem_ChannelsTest
* Brief:  Check whether TP can enter Factory Mode, and do some thing
* Input: none
* Output: none
* Return: Comm Code. Code = 0x00 is OK, else fail.
...
unsigned char FTE716_TestItem_ChannelsTest(bool * bTestResult)
{
	...
	if(ReCode == ERROR_CODE_OK)
	{
		if((g_stCfg_FTE716_BasicThreshold.ChannelNumTest_ChannelXNum == g_stSCapConfEx.ChannelXNum)
			&& (g_stCfg_FTE716_BasicThreshold.ChannelNumTest_ChannelYNum == g_stSCapConfEx.ChannelYNum)
			&& (g_stCfg_FTE716_BasicThreshold.ChannelNumTest_KeyNum == g_stSCapConfEx.KeyNum))
		{
			* bTestResult = true;
			FTS_TEST_DBG("\n\nGet channels: (CHx: %d, CHy: %d, Key: %d), Set channels: (CHx: %d, CHy: %d, Key: %d)",
				...
				g_stCfg_FTE716_BasicThreshold.ChannelNumTest_ChannelXNum, g_stCfg_FTE716_BasicThreshold.ChannelNumTest_ChannelYNum, g_stCfg_FTE716_BasicThreshold.ChannelNumTest_KeyNum);
...
/************************************************************************
* Name: FTE716_TestItem_RawDataTest
...
/************************************************************************
* Name: FTE716_TestItem_NoiseTest
...
unsigned char FTE716_TestItem_NoiseTest(bool* bTestResult)
{
	...
	iNoiseFrames = g_stCfg_FTE716_BasicThreshold.NoiseTest_Frames;
	...
	if(NT_AvgData == g_stCfg_FTE716_BasicThreshold.NoiseTest_NoiseMode)
	{
	...
	else if(NT_MaxData == g_stCfg_FTE716_BasicThreshold.NoiseTest_NoiseMode)
	{
	...
	else if(NT_MaxDevication == g_stCfg_FTE716_BasicThreshold.NoiseTest_NoiseMode)
	{
	...
	else if(NT_DifferData == g_stCfg_FTE716_BasicThreshold.NoiseTest_NoiseMode)
	{
		//Caculate the Avg Value of all nodes
		memset(m_NoiseData, 0, sizeof(m_NoiseData));
	...
	if( ReCode != ERROR_CODE_OK ) goto TEST_ERR;
		...
		iMinValue = 0;
		iMaxValue = g_stCfg_FTE716_BasicThreshold.NoiseTest_Coefficient * chNoiseValue * 32 / 100;
		FTS_TEST_DBG("");
		for(iRow = 0;iRow < (g_stSCapConfEx.ChannelXNum + 1);iRow++)
		{
			for(iCol = 0;iCol < g_stSCapConfEx.ChannelYNum;iCol++)
...
unsigned char FTE716_TestItem_CbTest(bool* bTestResult)
{
	...
	iMinValue = g_stCfg_FTE716_BasicThreshold.CbTest_Min;
	iMaxValue = g_stCfg_FTE716_BasicThreshold.CbTest_Max;	
	for(iRow = 0;iRow < (g_stSCapConfEx.ChannelXNum + 1);iRow++)
	{
		for(iCol = 0;iCol < g_stSCapConfEx.ChannelYNum;iCol++)
		{
