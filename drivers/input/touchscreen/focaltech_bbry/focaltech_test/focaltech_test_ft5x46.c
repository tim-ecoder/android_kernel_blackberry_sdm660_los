...
unsigned char FT5X46_TestItem_RawDataTest(bool * bTestResult)
{
	...
	if (g_ScreenSetParam.isNormalize == Auto_Normalize)
	{
		...
		FTS_TEST_DBG( "\n=========FIR State: ON");
		ReCode = WriteReg(0xFB, 1);//FIR OFF  0رգ1
		if(ReCode != ERROR_CODE_OK)		
		{
	...
	ReCode = WriteReg( REG_NORMALIZE_TYPE, OriginValue );//ָԭĴֵ
	if(ReCode != ERROR_CODE_OK)		
	{
		FTS_TEST_DBG("\n Write REG_NORMALIZE_TYPE error. Error Code: %d",  ReCode);
		goto TEST_ERR;
...
unsigned char FT5X46_TestItem_SCapCbTest(bool* bTestResult)
{
	...
	for(i = 0; i < 3; i++)
	{
		...
		//ˮCB
		ReCode = WriteReg( REG_ScWorkMode, 1 );//ݹʽѡ:  1ˮ 0:Ƿˮ
		if( ReCode != ERROR_CODE_OK )
		{
		...
		//Ƿˮrawdata
		ReCode = WriteReg( REG_ScWorkMode, 0 );//ݹʽѡ:  1ˮ 0:Ƿˮ
		if( ReCode != ERROR_CODE_OK )
		{
			FTS_TEST_DBG("Get REG_ScWorkMode Failed!");
			goto TEST_ERR;
...
static boolean GetTestCondition(int iTestType, unsigned char ucChannelValue)
{
	...
	switch(iTestType)
	{
	case WT_NeedProofOnTest://Bit5:  0ˮģʽ;  1ˮģʽ
		bIsNeeded = !( ucChannelValue & 0x20 );
		break;
	case WT_NeedProofOffTest://Bit7: 0 ͨģʽ⣻ 1ͨģʽ
...
//	strDsen = "Dsen:";
	...
	//	strDsen +=  strTemp;
		...
		if((2047+iDoffset) - iDsen <= 0)//Сڵ0ֱPASS
		{
			continue;
		}
	...
	//	strRes +=  strTemp;
		...
		if((iMin_CG > fGShortResistance[i]) || (iDsen - iDoffset < 0))//Сڵ0ֱ0ŷ·
		{
			iCount++;
			if(i+1 <= iMaxTx)
...
static unsigned char WeakShort_GetAdcData( int AllAdcDataLen, int *pRevBuffer  )
{
	...
	ReCode = WriteReg(0x07, 0x01);// ʹܺ΢·һ
	if(ReCode != ERROR_CODE_OK)
	{
...
unsigned char FT5X46_TestItem_PanelDifferTest(bool * bTestResult)
{
	...
	ReCode = ReadReg( REG_NORMALIZE_TYPE, &OriginRawDataType );//ȡԭʼֵ	
	if(ReCode != ERROR_CODE_OK)		
	{
	...
	ReCode =ReadReg( 0x0A, &OriginFrequecy);//ȡԭʼֵ	
	if( ReCode != ERROR_CODE_OK )
	{
	...
	ReCode = ReadReg( 0xFB, &OriginFirState);//ȡԭʼֵ	
	if( ReCode != ERROR_CODE_OK )
	{
...
//	SaveTestData(strTemp, m_iTxNum, m_iRxNum);
	...
	ReCode = WriteReg( REG_NORMALIZE_TYPE, OriginRawDataType );//ָԭĴֵ
	ReCode = WriteReg( 0x0A, OriginFrequecy );//ָԭĴֵ
	ReCode = WriteReg( 0xFB, OriginFirState );//ָԭĴֵ
