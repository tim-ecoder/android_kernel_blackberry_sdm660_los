...
static struct device_attribute attrs[] = {
	...
	/* MODIFIED-BEGIN by Haojun Chen, 2017-08-08,BUG-5159539*/
	__ATTR(cover_mode, (S_IRUGO | S_IWUSR | S_IWGRP),
...
static void tp_class_device_register(void)
{
	...
	tp_device_class = class_create(THIS_MODULE, "tp_device");
	if (IS_ERR(tp_device_class))
		pr_err("Failed to create class(tp_device_class)!\n");

	tp_gesture_dev = device_create(tp_device_class, NULL, 0, NULL, "tp_gesture");
	if (IS_ERR(tp_gesture_dev))
		pr_err("Failed to create device(tp_gesture)!\n");
...
/* MODIFIED-BEGIN by Haojun Chen, 2017-08-08,BUG-5159539*/
	...
	tp_cover_dev = device_create(tp_device_class, NULL, 0, NULL, "tp_cover");
	if (IS_ERR(tp_cover_dev))
		pr_err("Failed to create device(tp_glove)!\n");

	rc = device_create_file(tp_cover_dev, &dev_attr_cover_enable);
	if ( rc < 0)
...
static void synaptics_rmi4_f12_glove_enable(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	int retval;
	unsigned char offset;
	unsigned char f12_ctrl23;
	struct synaptics_rmi4_f12_extra_data *extra_data;
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
	...
	if (enable)
		f12_ctrl23 = f12_ctrl23 | 0x20;
	else
		f12_ctrl23 = f12_ctrl23 & 0xdf;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	synaptics_rmi4_f12_glove_enable(rmi4_data, input);

	return count;
}

/* MODIFIED-BEGIN by Haojun Chen, 2017-08-08,BUG-5159539*/
static void synaptics_rmi4_f12_cover_enable(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	int retval;
	unsigned char data;
	...
	retval = synaptics_rmi4_reg_write(rmi4_data, 0x001e, &data, sizeof(data));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	input = input > 0 ? 1 : 0;
	rmi4_data->cover_mode = input;
	synaptics_rmi4_f12_cover_enable(rmi4_data, input);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (detected_gestures) {
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_U, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_U, 0);
			input_sync(rmi4_data->input_dev);
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		set_bit(KEY_GESTURE_U, rmi4_data->input_dev->keybit);
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->input_dev->id.vendor = 0x0001;
		...
		bool get)
{
	...
	if ((bdata->bus_reg_name != NULL) && (*bdata->bus_reg_name != 0)) {
		...
		if (regulator_count_voltages(rmi4_data->bus_reg) > 0) {
			...
			if (retval) {
				dev_err(rmi4_data->pdev->dev.parent,
					"Regulator set_vtg failed %s rc=%d", bdata->bus_reg_name, retval);
				goto regulator_put;
			}
		}
	}
#if 1
	rmi4_data->disp_n5v = regulator_get(rmi4_data->pdev->dev.parent, "disp_n5v");
	if (IS_ERR(rmi4_data->disp_n5v)) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to get disp_n5v regulator\n",
					__func__);
			retval = PTR_ERR(rmi4_data->disp_n5v);
			goto regulator_put;
	}
#if 0
	/* MODIFIED-BEGIN by Haojun Chen, 2017-08-30,BUG-5191985*/
	retval = regulator_set_voltage(rmi4_data->disp_n5v, 5800000, 5800000);
	if (retval) {
		dev_err(rmi4_data->pdev->dev.parent,
			"Regulator set_vtg failed %s rc=%d", "disp_n5v", retval);
		goto regulator_put;
		/* MODIFIED-END by Haojun Chen,BUG-5191985*/
	}
#endif
	rmi4_data->disp_p5v = regulator_get(rmi4_data->pdev->dev.parent, "disp_p5v");
	if (IS_ERR(rmi4_data->disp_p5v)) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to get disp_p5v regulator\n",
					__func__);
			retval = PTR_ERR(rmi4_data->disp_p5v);
			goto regulator_put;
	}
#if 0
	/* MODIFIED-BEGIN by Haojun Chen, 2017-08-30,BUG-5191985*/
	retval = regulator_set_voltage(rmi4_data->disp_p5v, 5800000, 5800000);
	if (retval) {
		dev_err(rmi4_data->pdev->dev.parent,
			"Regulator set_vtg failed %s rc=%d", "disp_p5v", retval);
		goto regulator_put;
		/* MODIFIED-END by Haojun Chen,BUG-5191985*/
	}
#endif
...
#if 1
	if (rmi4_data->disp_n5v) {
		regulator_put(rmi4_data->disp_n5v);
		rmi4_data->disp_n5v = NULL;
	}
	if (rmi4_data->disp_p5v) {
		regulator_put(rmi4_data->disp_p5v);
		rmi4_data->disp_p5v = NULL;
	}
#endif
	return retval;
}
		...
		bool enable)
{
	...
	if (enable) {
		if (rmi4_data->bus_reg) {
			...
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to enable bus pullup regulator\n", __func__);
			}
		}
		...
		if (rmi4_data->pwr_reg) {
			...
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to enable power regulator\n", __func__);
			}
			msleep(bdata->power_delay_ms);
		}

		if (rmi4_data->disp_n5v) {
			retval = regulator_enable(rmi4_data->disp_n5v);
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to enable disp_n5v regulator\n", __func__);
			}
		}

		if (rmi4_data->disp_p5v) {
			retval = regulator_enable(rmi4_data->disp_p5v);
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to enable disp_p5v regulator\n", __func__);
			}
		}
	} else {
		if (rmi4_data->bus_reg) {
			...
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to disable bus pullup regulator\n", __func__);
			}
		}
		...
		if (rmi4_data->pwr_reg) {
			...
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to disable power regulator\n", __func__);
			}
			msleep(bdata->power_delay_ms);
		}

		if (rmi4_data->disp_n5v) {
			retval = regulator_disable(rmi4_data->disp_n5v);
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to disable disp_n5v regulator\n", __func__);
			}
		}

		if (rmi4_data->disp_n5v) {
			retval = regulator_disable(rmi4_data->disp_p5v);
			if (retval < 0) {
				dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to disable disp_n5v regulator\n", __func__);
			}
		}
		...
		unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && rmi4_data) {
		if (event == FB_EARLY_EVENT_BLANK && *transition == FB_BLANK_POWERDOWN) {
			if (rmi4_data->enable_wakeup_gesture && rmi4_data->state == TP_STATE_ON) {
				printk("synaptics on -> gesture FB_EARLY_EVENT_BLANK \n");
				synaptics_rmi4_suspend(&rmi4_data->pdev->dev);
			}
		} else if (event == FB_EVENT_BLANK && *transition == FB_BLANK_POWERDOWN) {
			if (!bb_panel_is_off) {
				printk("[mdss] bb_panel_is_off = %d", bb_panel_is_off);
				return 0;
			}
			if (rmi4_data->enable_wakeup_gesture) {
				printk("synaptics on -> gesture FB_EVENT_BLANK \n");
				rmi4_data->state = TP_STATE_GESTURE;
			} else {
				printk("synaptics on -> off  FB_EVENT_BLANK \n");
				synaptics_rmi4_suspend(&rmi4_data->pdev->dev);
				usleep(5000);
				if (rmi4_data->disp_n5v) {
					retval = regulator_disable(rmi4_data->disp_n5v);
				...
				rmi4_data->state = TP_STATE_OFF;
			}
			rmi4_data->fb_ready = false;
		} else if (event == FB_EARLY_EVENT_BLANK && *transition == FB_BLANK_UNBLANK) {
			if(rmi4_data->state == TP_STATE_OFF) {
				printk("synaptics off -> on \n");
				...
				printk("synaptics gesture -> on FB_EARLY_EVENT_BLANK\n");
				gpio_set_value(bdata->reset_gpio, 0);
				usleep(10000);
				gpio_set_value(bdata->reset_gpio, 1);
				usleep(10000);
			} else if (rmi4_data->state == TP_STATE_IGNORE_TOUCH) {
				synaptics_rmi4_wakeup_gesture(rmi4_data, false);
				printk("synaptics ignore touch -> on FB_EARLY_EVENT_BLANK\n");
			}
		}else if (event == FB_EVENT_BLANK && *transition == FB_BLANK_UNBLANK) {
			if (rmi4_data->state == TP_STATE_GESTURE || rmi4_data->state == TP_STATE_IGNORE_TOUCH) {
				usleep(10000);
				synaptics_rmi4_resume(&rmi4_data->pdev->dev);
			}
			rmi4_data->state = TP_STATE_ON;
			rmi4_data->fb_ready = true;
		} else if (event == FB_EARLY_EVENT_BLANK && *transition == FB_BLANK_NORMAL) {
			if (rmi4_data->state == TP_STATE_OFF) {
				printk("synaptics off -> ignore touch \n");
				gpio_set_value(bdata->reset_gpio, 0);
				usleep(10000);
				gpio_set_value(bdata->reset_gpio, 1);
				if (rmi4_data->disp_p5v) {
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->suspend) {
		...
		usleep(TDDI_LPWG_WAIT_US);
#endif
		synaptics_rmi4_f12_wg(rmi4_data, true);
		synaptics_rmi4_irq_enable(rmi4_data, false, false);
		synaptics_rmi4_sleep_enable(rmi4_data, true);
		synaptics_rmi4_free_fingers(rmi4_data);
	}
...
/* MODIFIED-BEGIN by Haojun Chen, 2017-08-30,BUG-5191985*/
static void synaptics_rmi4_shutdown(struct platform_device *pdev)
{
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(pdev);
	const struct synaptics_dsx_board_data *bdata =
		rmi4_data->hw_if->board_data;
...
static struct platform_driver synaptics_rmi4_driver = {
	...
	.shutdown = synaptics_rmi4_shutdown, // MODIFIED by Haojun Chen, 2017-08-30,BUG-5191985
};

static int __init synaptics_rmi4_init_2(void)
{
	int retval;
...
static void __exit synaptics_rmi4_exit_2(void)
{
...
module_init(synaptics_rmi4_init_2);
module_exit(synaptics_rmi4_exit_2);
